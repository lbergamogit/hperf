...
tab = table((1:6)', P_total / units.kilowatt, P_ind / units.kilowatt, ...
    P_profile / units.kilowatt, P_parasite / units.kilowatt, ...
    P_climb / units.kilowatt, P_descent / units.kilowatt, ...
    P_misc / units.kilowatt, speed / units.knot, fuel / 0.8);
tab.Properties.VariableNames = {'Phase', 'P_total', 'P_induced', ...
    'P_profile', 'P_parasite','P_climb', 'P_descent','P_miscellaneous', ...
    'Speed', 'Fuel'};
tab.Properties.VariableUnits = {'', 'kW', 'kW', 'kW', 'kW', 'kW', 'kW', ...
    'kW', 'kt', 'l'};
disp(tab)
writetable(tab, 'mission_results.txt');

total_time = sum(time);
hh = floor(total_time / 3600);
mm = floor((total_time - hh * 3600) / 60);
ss = floor(total_time - hh * 3600 - mm * 60);
fprintf('Total fuel: %.1f l\n', sum(fuel) / 0.8)
